...
import {
  ...
  MockSupportVar,
  SUPPORT_VAR_FAIL,
  SUPPORT_VAR_SUCCCESS
} from 'Support/tests/mocks';
...
describe('Contact Form Validation', () => {
  ...
  it('Topic field has filled', async () => {
    ...
    fireEvent.change(topicInput, {
      target: { value: t.support.topicOther }
    });
    fireEvent.click(submitButton);
    await waitFor(() => new Promise((res) => setTimeout(res, 1)));
...
const fillSupportFormInputs = (
  utils: TestRender,
  variables: MockSupportVar
) => {
...
describe('Support Form - Submit', () => {
  it('Should expect error message when onError', async () => {
    ...
    fillSupportFormInputs(utils, SUPPORT_VAR_FAIL);
  ...
  it('Should expect success message when submission succeeds', async () => {
    const utils = render(
      <SnackbarProvider>
        <ContactForm />
      </SnackbarProvider>,
      { mocks: [mockSupportSuccess] }
    );

    fillSupportFormInputs(utils, SUPPORT_VAR_SUCCCESS);
    await waitFor(() => new Promise((res) => setTimeout(res, 0)));
    fireEvent.click(utils.getByTestId('sendButton'));
    await waitFor(() => new Promise((res) => setTimeout(res, 0)));
