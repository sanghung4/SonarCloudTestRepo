create
or replace view MORSCO_EDW.INNOVATE_TM_SCHEMA.INNOVATION_PRODUCT_MAX(
	INTERNAL_ITEM_NBR,
	ID,
	ERP_PRODUCT_ID,
	BUYLINEID,
...
cte_pdwmeta AS (
	SELECT
		...
		description
	FROM
		morsco_edw.stage.pdwmeta QUALIFY ROW_NUMBER() OVER(
...
price_line_by_branch AS (
	...
	FROM
		morsco_edw.stage.pricelineauthbranch
),
exclusion_mapping AS (
	SELECT
		...
		CASE
			WHEN metaid = 426
			AND metavalue = 1 THEN 'al'
			WHEN metaid = 427
			AND metavalue = 1 THEN 'az'
			WHEN metaid = 428
			AND metavalue = 1 THEN 'ar'
			WHEN metaid = 429
			AND metavalue = 1 THEN 'ca'
			WHEN metaid = 430
			AND metavalue = 1 THEN 'co'
			WHEN metaid = 431
			AND metavalue = 1 THEN 'ct'
			WHEN metaid = 432
			AND metavalue = 1 THEN 'de'
			WHEN metaid = 433
			AND metavalue = 1 THEN 'dc'
			WHEN metaid = 434
			AND metavalue = 1 THEN 'fl'
			WHEN metaid = 435
			AND metavalue = 1 THEN 'ga'
			WHEN metaid = 436
			AND metavalue = 1 THEN 'id'
			WHEN metaid = 437
			AND metavalue = 1 THEN 'il'
			WHEN metaid = 438
			AND metavalue = 1 THEN 'in'
			WHEN metaid = 439
			AND metavalue = 1 THEN 'ia'
			WHEN metaid = 440
			AND metavalue = 1 THEN 'ks'
			WHEN metaid = 441
			AND metavalue = 1 THEN 'ky'
			WHEN metaid = 442
			AND metavalue = 1 THEN 'la'
			WHEN metaid = 443
			AND metavalue = 1 THEN 'md'
			WHEN metaid = 444
			AND metavalue = 1 THEN 'md'
			WHEN metaid = 445
			AND metavalue = 1 THEN 'ma'
			WHEN metaid = 446
			AND metavalue = 1 THEN 'mi'
			WHEN metaid = 447
			AND metavalue = 1 THEN 'mn'
			WHEN metaid = 448
			AND metavalue = 1 THEN 'ms'
			WHEN metaid = 449
			AND metavalue = 1 THEN 'mo'
			WHEN metaid = 450
			AND metavalue = 1 THEN 'mt'
			WHEN metaid = 451
			AND metavalue = 1 THEN 'ne'
			WHEN metaid = 452
			AND metavalue = 1 THEN 'nv'
			WHEN metaid = 453
			AND metavalue = 1 THEN 'nh'
			WHEN metaid = 454
			AND metavalue = 1 THEN 'nj'
			WHEN metaid = 455
			AND metavalue = 1 THEN 'nm'
			WHEN metaid = 456
			AND metavalue = 1 THEN 'ny'
			WHEN metaid = 457
			AND metavalue = 1 THEN 'nc'
			WHEN metaid = 458
			AND metavalue = 1 THEN 'nd'
			WHEN metaid = 459
			AND metavalue = 1 THEN 'oh'
			WHEN metaid = 460
			AND metavalue = 1 THEN 'ok'
			WHEN metaid = 461
			AND metavalue = 1 THEN 'or'
			WHEN metaid = 462
			AND metavalue = 1 THEN 'pa'
			WHEN metaid = 463
			AND metavalue = 1 THEN 'ri'
			WHEN metaid = 464
			AND metavalue = 1 THEN 'sc'
			WHEN metaid = 465
			AND metavalue = 1 THEN 'sd'
			WHEN metaid = 466
			AND metavalue = 1 THEN 'tn'
			WHEN metaid = 467
			AND metavalue = 1 THEN 'tx'
			WHEN metaid = 468
			AND metavalue = 1 THEN 'ut'
			WHEN metaid = 469
			AND metavalue = 1 THEN 'vt'
			WHEN metaid = 470
			AND metavalue = 1 THEN 'va'
			WHEN metaid = 471
			AND metavalue = 1 THEN 'wa'
			WHEN metaid = 472
			AND metavalue = 1 THEN 'wv'
			WHEN metaid = 473
			AND metavalue = 1 THEN 'wi'
			WHEN metaid = 474
			AND metavalue = 1 THEN 'wy'
...
cte_territory_exclusion_list AS (
	SELECT
		...
		ARRAY_AGG(
			...
			ORDER BY
				exclusion_mapping.state_short_name
		) AS territory_exclusion_list
	from
		cte_pdwcatalog
...
cte_excl_branch AS (
	SELECT
		...
		pricelineid,
		(
			ARRAY_AGG(authbranchid) WITHIN GROUP (
				ORDER BY
					etlsourceid,
					pricelineid,
					authbranchid
...
cte_matrixbasisinfo AS (
	SELECT
		...
		ROW_NUMBER() OVER (
			PARTITION BY morsco_edw.ods.matrixbasisinfo.etlsourceid,
			COALESCE(morsco_edw.ods.matrixbasisinfo.branchid, ''),
			...
			COALESCE(
				morsco_edw.ods.matrixbasisinfo.prodsellgroupid,
				''
			)
			ORDER BY
				morsco_edw.ods.matrixbasisinfo.effectivedate DESC
		) AS rownum
	FROM
		morsco_edw.ods.matrixbasisinfo
		LEFT JOIN morsco_edw.ods.matrixdates ON morsco_edw.ods.matrixdates.etlsourceid = morsco_edw.ods.matrixbasisinfo.etlsourceid
		AND morsco_edw.ods.matrixdates.matrixdatesid || '~' || CAST(
			DATEDIFF(
				DD,
				'1967-12-31',
				CAST(morsco_edw.ods.matrixdates.effectivedate AS DATE)
			) AS VARCHAR
...
product_pricing AS (
	...
	FROM
		morsco_edw.stage.productpricing
	WHERE
		effectivedate <= current_date() QUALIFY row_number() OVER (
			PARTITION BY etlsourceid,
			productid
			ORDER BY
				effectivedate DESC,
				loaddt DESC,
				competitivemarketprice desc
		) = 1
),
/*
...
latest_product AS (
	...
	WHERE
		...
		AND NOT (
			TRIM(morsco_edw.stage.product.proddesc) LIKE ANY(UPPER('%<D>%'), '**%', '%�%')
		)
		AND (
			UPPER(morsco_edw.stage.product.indextype) = 'P'
...
products_with_pricing AS (
	SELECT
		latest_product.*,
		COALESCE(
			cmp_matrix_basis_info_prod.basisnumber,
			cmp_matrix_basis_info_sell.basisnumber,
			all_matrix_basis_info_prod.basisnumber,
			all_matrix_basis_info_sell.basisnumber
		) AS basis_number,
		COALESCE(
			cmp_matrix_basis_info_prod.formula,
			cmp_matrix_basis_info_sell.formula,
			all_matrix_basis_info_prod.formula,
			all_matrix_basis_info_sell.formula
		) AS mbi_formula,
		COALESCE(
			cmp_matrix_basis_info_prod.multiplier,
			cmp_matrix_basis_info_sell.multiplier,
			all_matrix_basis_info_prod.multiplier,
			all_matrix_basis_info_sell.multiplier
		) AS multiplier,
		...
		CASE
			...
			AND (
				LEFT(mbi_formula, 1) = '*'
				OR LEFT(mbi_formula, 1) = 'X'
			) THEN cmp * formula_price
			WHEN basis_number = '4'
			AND LEFT(mbi_formula, 1) = 'D' THEN cmp / formula_price
			WHEN basis_number = '4'
			AND LEFT(mbi_formula, 2) = 'GP' THEN cmp / gp_calc
			ELSE cmp
		END AS calculated_cmp
	FROM
		latest_product
		LEFT JOIN umsp_matrix_basis_info as umsp_matrix_basis_info_prod ON latest_product.productid = umsp_matrix_basis_info_prod.productid
		AND latest_product.etlsourceid = umsp_matrix_basis_info_prod.etlsourceid
		LEFT JOIN cmp_matrix_basis_info as cmp_matrix_basis_info_prod ON latest_product.productid = cmp_matrix_basis_info_prod.productid
		AND latest_product.etlsourceid = cmp_matrix_basis_info_prod.etlsourceid
		LEFT JOIN cmp_matrix_basis_info as cmp_matrix_basis_info_sell ON latest_product.pricelineid = cmp_matrix_basis_info_sell.prodsellgroupid
		AND latest_product.etlsourceid = cmp_matrix_basis_info_sell.etlsourceid
		LEFT JOIN all_matrix_basis_info as all_matrix_basis_info_prod ON latest_product.productid = all_matrix_basis_info_prod.productid
		AND latest_product.etlsourceid = all_matrix_basis_info_prod.etlsourceid
		LEFT JOIN all_matrix_basis_info as all_matrix_basis_info_sell ON latest_product.pricelineid = all_matrix_basis_info_sell.prodsellgroupid
		AND latest_product.etlsourceid = all_matrix_basis_info_sell.etlsourceid
		AND all_matrix_basis_info_sell.productid IS NULL
		LEFT JOIN product_pricing ON latest_product.etlsourceid = product_pricing.etlsourceid
		AND latest_product.productid = product_pricing.productid
),
/*
...
sold_products AS (
	...
	FROM
		morsco_edw.ods.ledgersoheader
		INNER JOIN morsco_edw.ods.ledgersodetail ON morsco_edw.ods.ledgersoheader.etlsourceid = morsco_edw.ods.ledgersodetail.etlsourceid
	...
	WHERE
		morsco_edw.ods.ledgersoheader.etlsourceid = 'MSC'
...
product_stock AS (
	SELECT
		productid,
		etlsourceid,
		ARRAY_AGG(branchid) WITHIN GROUP (
			ORDER BY
				productid,
				etlsourceid
		) AS in_stock_location_list
	FROM
		morsco_edw.ods.productdynam
	WHERE
		etlsourceid = 'MSC'
		AND availableqty > 0
	GROUP BY
...
cte_product AS (
	SELECT
		...
		IFF(
			...
			'"' || 'in_stock_location' || '"' || ': ' || CAST(product_stock.in_stock_location_list AS VARCHAR),
			NULL
		) AS in_stock_location_list,
		IFF(
			cte_excl_branch.product_branch_exclusion IS NOT NULL,
			'"' || 'product_branch_exclusion' || '"' || ': ' || CAST(
				cte_excl_branch.product_branch_exclusion AS VARCHAR
			),
			NULL
		) AS product_branch_exclusion
	FROM
		products_with_pricing --Key Products
		INNER JOIN morsco_edw.stage.key_product ON products_with_pricing.etlsourceid = morsco_edw.stage.key_product.etlsourceid
		AND products_with_pricing.productid = morsco_edw.stage.key_product.productid --Instock Products
		LEFT JOIN product_stock ON products_with_pricing.productid = product_stock.productid
		AND products_with_pricing.etlsourceid = product_stock.etlsourceid --Product sold count
		LEFT JOIN sold_products ON products_with_pricing.etlsourceid = sold_products.etlsourceid
		AND products_with_pricing.productid = sold_products.productid --List of Branches excluded
		LEFT JOIN cte_excl_branch ON products_with_pricing.etlsourceid = cte_excl_branch.etlsourceid
...
cte_entitypnids AS (
	SELECT
		...
		loaddt AS last_changed_date
	FROM
		morsco_edw.stage.entitypnids
	WHERE
		customerpartnum NOT LIKE '%\u008D%' QUALIFY ROW_NUMBER() OVER(
			PARTITION BY entitypnid
			ORDER BY
				loaddt DESC
		) = 1
),
...
cte_entity_objects AS (
	SELECT
		...
		ARRAY_AGG(cte_entitypnids.customerpartnum) WITHIN GROUP (
			ORDER BY
				cte_product.etlsourceid,
				cte_product.productid,
				cte_entitypnids.entityid
	...
	FROM
		cte_entitypnids
		INNER JOIN cte_product ON cte_product.erpproductid = cte_entitypnids.eclipsepartnum
...
cte_customer_pn AS (
	SELECT
		...
		'"' || 'customer_number' || '":' || CAST(
			ARRAY_AGG(cte_entity_objects.entityid) WITHIN GROUP (
				ORDER BY
					cte_entity_objects.etlsourceid,
					cte_entity_objects.productid
			) AS VARCHAR
		) as customer_nbr_list,
		'"' || 'customer_part_nbr_list' || '":' || CAST(
			ARRAY_UNION_AGG(cte_entity_objects.parts_array) AS VARCHAR
		) as customer_part_nbr_list,
		'"' || 'customer_part_numbers' || '":' || CAST(
			ARRAY_AGG(cte_entity_objects.cust_parts_dict) AS VARCHAR
		) as customer_part_numbers,
...
product_field_values AS (
	SELECT
		...
		NULL AS value,
		CAST(
			REPLACE(description, 'TS ATTRIB NAME', '') AS INT
		) AS ts_seq
	FROM
		cte_pdwmeta
	WHERE
		description LIKE 'TS ATTRIB NAME%'
		AND fieldid IN (
			'82',
			'83',
			'84',
	...
	SELECT
		...
		'VALUE' AS value,
		CAST(
			REPLACE(description, 'TS ATTRIB VALUE', '') AS INT
		) AS ts_seq
	FROM
		cte_pdwmeta
	WHERE
		description LIKE 'TS ATTRIB VALUE%'
		AND fieldid IN (
			'131',
			'132',
			'133',
...
product_tech_specs AS (
	...
	FROM
		cte_pdwcatalog
		INNER JOIN cte_product ON cte_pdwcatalog.pdwid = cte_product.pdwid
		AND cte_pdwcatalog.etlsourceid = cte_product.etlsourceid --ASSEMBLE THE NAME AND VALUE PAIR TO BE ABLE TO JOIN UP INTO A LIST
		INNER JOIN distinct_product_field_values ON cte_pdwcatalog.metaid = distinct_product_field_values.fieldid
	GROUP BY
		1,
		2,
		3,
		4
	HAVING
		IFNULL(ts_nested_value, ' ') > ' '
		AND IFNULL(ts_value, ' ') > ' ' QUALIFY ROW_NUMBER() OVER(
			PARTITION BY cte_product.productid,
			ts_label
			ORDER BY
				distinct_product_field_values.ts_seq
		) = 1
...
cte_tech_specs AS (
	SELECT
		...
		'"' || 'technical_specifications' || '"' || ': ' || '[{' || (
			LISTAGG(DISTINCT ts_nested_value, ',') WITHIN GROUP (
				ORDER BY
					ts_nested_value
...
products_list AS (
	SELECT
		...
		MAX(
			IFF(
				cte_pdwcatalog.metaid = '3',
				cte_pdwcatalog.metavalue,
				cte_product.catalognumber
			)
		) AS vendor_part_nbr,
		MAX(
			IFF(
				cte_pdwcatalog.metaid = '70059',
				cte_pdwcatalog.metavalue,
				cte_product.upc
			)
		) AS upc_id,
		MAX(
			CASE
				WHEN cte_pdwcatalog.metaid = '70052' THEN cte_pdwcatalog.metavalue
			END
		) AS unspc_id,
		-- SEARCH KEYWORDS
		...
		MAX(
			CASE
				WHEN cte_pdwcatalog.metaid = '423' THEN cte_pdwcatalog.metavalue
			END
		) AS category_1_name,
		MAX(
			CASE
				WHEN cte_pdwcatalog.metaid = '424' THEN cte_pdwcatalog.metavalue
			END
		) AS category_2_name,
		MAX(
			CASE
				WHEN cte_pdwcatalog.metaid = '425' THEN cte_pdwcatalog.metavalue
			END
		) AS category_3_name,
		...
		MAX(
			CASE
				WHEN cte_pdwcatalog.metaid = '70004' THEN cte_pdwcatalog.metavalue
			END
		) AS product_overview_description,
		-- MANUFACTURER INFO
		MAX(
			CASE
				WHEN cte_pdwcatalog.metaid = '70024' THEN cte_pdwcatalog.metavalue
			END
		) AS mfr_full_name,
		-- TECHNICAL, FEATURES AND BENEFITS
		MAX(
			CASE
				WHEN cte_pdwcatalog.metaid = '217' THEN cte_pdwcatalog.metavalue
			END
		) AS feature_benefit_list_text,
		-- IMAGES
		MAX(
			CASE
				WHEN cte_pdwcatalog.metaid = '70050' THEN cte_pdwcatalog.metavalue
			END
		) AS thumbnail_image_url_name,
		MAX(
			CASE
				WHEN cte_pdwcatalog.metaid = '234' THEN cte_pdwcatalog.metavalue
			END
		) AS medium_image_url_name,
		MAX(
			CASE
				WHEN cte_pdwcatalog.metaid = '70012' THEN cte_pdwcatalog.metavalue
			END
		) AS full_image_url_name,
		-- TECHNICAL DOCUMENTS
		MAX(
			CASE
				WHEN cte_pdwcatalog.metaid = '291' THEN cte_pdwcatalog.metavalue
			END
		) AS mfr_catalog_doc_file_name,
		MAX(
			CASE
				WHEN cte_pdwcatalog.metaid = '292' THEN cte_pdwcatalog.metavalue
			END
		) AS mfr_spec_tech_doc_file_name,
		MAX(
			CASE
				WHEN cte_pdwcatalog.metaid = '238' THEN cte_pdwcatalog.metavalue
			END
		) AS mfr_msds_doc_file_name,
		MAX(
			CASE
				WHEN cte_pdwcatalog.metaid = '226' THEN cte_pdwcatalog.metavalue
			END
		) AS mfr_install_instruction_doc_file_name,
		MAX(
			CASE
				WHEN cte_pdwcatalog.metaid = '236' THEN cte_pdwcatalog.metavalue
			END
		) AS mfr_item_data_doc_file_name,
		-- ENVIRONMENTAL OPTIONS
		MAX(
			CASE
				WHEN cte_pdwcatalog.metaid = '232' THEN cte_pdwcatalog.metavalue
			END
		) AS low_lead_compliant_flag,
		MAX(
			CASE
				WHEN cte_pdwcatalog.metaid = '235' THEN cte_pdwcatalog.metavalue
			END
		) AS mercury_free_flag,
		MAX(
			CASE
				WHEN cte_pdwcatalog.metaid = '286' THEN cte_pdwcatalog.metavalue
			END
		) AS water_sense_compliant_flag,
		MAX(
			CASE
				WHEN cte_pdwcatalog.metaid = '215' THEN cte_pdwcatalog.metavalue
			END
		) AS energy_star_flag,
		MAX(
			CASE
				WHEN cte_pdwcatalog.metaid = '221' THEN cte_pdwcatalog.metavalue
			END
		) AS hazardous_material_flag,
		-- PACKAGE DIMENSIONS
		MAX(
			CASE
				WHEN cte_pdwcatalog.metaid = '249' THEN cte_pdwcatalog.metavalue
			END
		) AS package_weight_nbr,
		MAX(
			CASE
				WHEN cte_pdwcatalog.metaid = '70101' THEN cte_pdwcatalog.metavalue
			END
		) AS package_width_nbr,
		MAX(
			CASE
				WHEN cte_pdwcatalog.metaid = '70102' THEN cte_pdwcatalog.metavalue
			END
		) AS package_length_nbr,
		MAX(
			CASE
				WHEN cte_pdwcatalog.metaid = '70103' THEN cte_pdwcatalog.metavalue
			END
		) AS package_height_nbr,
		MAX(
			CASE
				WHEN cte_pdwcatalog.metaid = '70104' THEN cte_pdwcatalog.metavalue
			END
		) AS package_volume_nbr,
		MAX(
			CASE
				WHEN cte_pdwcatalog.metaid = '70107' THEN cte_pdwcatalog.metavalue
			END
		) AS package_volume_uom_code,
		MAX(
			CASE
				WHEN cte_pdwcatalog.metaid = '255' THEN cte_pdwcatalog.metavalue
			END
		) AS package_weight_uom_code,
		-- MINIMUM INCREMENT QUANTITY
		MAX(
			CASE
				WHEN cte_pdwcatalog.metaid = '317' THEN cte_pdwcatalog.metavalue
			END
		) AS minimum_increment_qty,
		-- BOOST SEARCH VALUES
		MAX(
			CASE
				WHEN cte_pdwcatalog.metaid = '172'
				AND IFNULL(
					cte_pdwcatalog.metavalue,
					'0'
				) != '0' THEN cte_pdwcatalog.metavalue
			END
		) AS product_search_boost_1_nbr,
		MAX(
			CASE
				WHEN cte_pdwcatalog.metaid = '173'
				AND IFNULL(
					cte_pdwcatalog.metavalue,
					'0'
				) != '0' THEN cte_pdwcatalog.metavalue
		...
		MAX(
			CASE
				WHEN cte_pdwcatalog.metaid = '174'
				AND IFNULL(
					cte_pdwcatalog.metavalue,
					'0'
				) != '0' THEN cte_pdwcatalog.metavalue
		...
		MAX(
			CASE
				WHEN cte_pdwcatalog.metaid = '176'
				AND IFNULL(
					cte_pdwcatalog.metavalue,
					'0'
				) != '0' THEN cte_pdwcatalog.metavalue
		...
		MAX(
			CASE
				WHEN cte_pdwcatalog.metaid = '70094' THEN cte_pdwcatalog.metavalue
			END
		) AS product_line,
		MAX(
			CASE
				WHEN cte_pdwcatalog.metaid = '389' THEN cte_pdwcatalog.metavalue
			END
		) AS material,
		MAX(
			CASE
				WHEN cte_pdwcatalog.metaid = '390' THEN cte_pdwcatalog.metavalue
			END
		) AS color_finish,
		MAX(
			CASE
				WHEN cte_pdwcatalog.metaid = '391' THEN cte_pdwcatalog.metavalue
			END
		) AS size,
		MAX(
			CASE
				WHEN cte_pdwcatalog.metaid = '392' THEN cte_pdwcatalog.metavalue
			END
		) AS length,
		MAX(
			CASE
				WHEN cte_pdwcatalog.metaid = '393' THEN cte_pdwcatalog.metavalue
			END
		) AS width,
		MAX(
			CASE
				WHEN cte_pdwcatalog.metaid = '394' THEN cte_pdwcatalog.metavalue
			END
		) AS height,
		MAX(
			CASE
				WHEN cte_pdwcatalog.metaid = '395' THEN cte_pdwcatalog.metavalue
			END
		) AS depth,
		MAX(
			CASE
				WHEN cte_pdwcatalog.metaid = '396' THEN cte_pdwcatalog.metavalue
			END
		) AS voltage,
		MAX(
			CASE
				WHEN cte_pdwcatalog.metaid = '397' THEN cte_pdwcatalog.metavalue
			END
		) AS tonnage,
		MAX(
			CASE
				WHEN cte_pdwcatalog.metaid = '398' THEN cte_pdwcatalog.metavalue
			END
		) AS btu,
		MAX(
			CASE
				WHEN cte_pdwcatalog.metaid = '399' THEN cte_pdwcatalog.metavalue
			END
		) AS pressure_rating,
		MAX(
			CASE
				WHEN cte_pdwcatalog.metaid = '400' THEN cte_pdwcatalog.metavalue
			END
		) AS temperature_rating,
		MAX(
			CASE
				WHEN cte_pdwcatalog.metaid = '402' THEN cte_pdwcatalog.metavalue
			END
		) AS inlet_size,
		MAX(
			CASE
				WHEN cte_pdwcatalog.metaid = '403' THEN cte_pdwcatalog.metavalue
			END
		) AS flow_rate,
		MAX(
			CASE
				WHEN cte_pdwcatalog.metaid = '404' THEN cte_pdwcatalog.metavalue
			END
		) AS capacity,
		MAX(
			CASE
				WHEN cte_pdwcatalog.metaid = '405' THEN cte_pdwcatalog.metavalue
			END
		) AS wattage,
		...
		MAX(
			cte_territory_exclusion_list.territory_exclusion_list :: varchar
		) AS territory_exclusion_list
	FROM
		cte_product
		LEFT JOIN cte_pdwcatalog ON cte_product.pdwid = cte_pdwcatalog.pdwid
		LEFT JOIN cte_tech_specs ON cte_product.pdwid = cte_tech_specs.pdwid
		AND cte_product.productid = cte_tech_specs.productid
		AND cte_product.etlsourceid = cte_tech_specs.etlsourceid
		LEFT JOIN cte_customer_pn ON cte_product.pdwid = cte_customer_pn.pdwid
		AND cte_product.productid = cte_customer_pn.productid
		AND cte_product.etlsourceid = cte_customer_pn.etlsourceid
		LEFT JOIN cte_territory_exclusion_list ON cte_product.pdwid = cte_territory_exclusion_list.pdwid
	GROUP BY
		1,
		2
)
